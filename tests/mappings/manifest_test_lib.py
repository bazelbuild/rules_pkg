...
class ContentManifestTest(unittest.TestCase):
  ...
  def _read_manifest(cls, path):
    ...
    return {x['dest']: x for x in json.loads(raw.decode('utf-16-le' if raw[1:2] == b'\0' else 'utf-8'))}
  ...
  def assertManifestsMatch(self, expected_path, got_path):
    ...
    """
    expected_dict = self._read_manifest(expected_path)
    got_dict = self._read_manifest(got_path)
